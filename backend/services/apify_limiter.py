...
DEFAULT_APIFY_RUN_SLOT_LIMIT = 20
...
def _rpc_claim(
    ...
    *,
    ...
    r = supabase.rpc(
        ...
        {
            "p_max_slots": DEFAULT_APIFY_RUN_SLOT_LIMIT,
...
def apify_run_slot(settings: Settings, actor_id: str) -> Iterator[None]:
    ...
    if not url or not key:
        logger.warning(
            ...
            "(dev-only; configure Supabase in deployed API/worker services)"
        )
        yield
        return
    ...
    if slot is None:
        raise ApifySlotWaitTimeout(
            ...
            f"(max_slots={DEFAULT_APIFY_RUN_SLOT_LIMIT}, actor={actor_id}). "
